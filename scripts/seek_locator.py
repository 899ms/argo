#!/usr/bin/env python3
"""seek_locator.py — local-seek 子技能的「安装感知」定位（唯一来源，不写死主机路径）。
...
查找顺序（命中即返回，全部未命中则返回打包位置，由调用方据此 fail 提示）：
  ...
  2. <ARGO_ROOT>/sub-skills/local-seek/scripts/seek.py
                               —— 打包子技能，锚点为本模块的安装位置（symlink 会解析回来源）
