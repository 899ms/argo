...
> v2.8.8 ships all of the above: 227 sources, 90 domains, 189 keyless. Details in [docs/为什么选择argo.md](docs/为什么选择argo.md).
...
- **227 sources (189 usable with no key), 90 domains**: general web + finance / macro / film / sports / geo / orgs / media / chemistry / academic / code (source of truth: `config.yaml` + `engines/specs/*.yaml`)
...
Config currently has **227** sources (**189** usable with no key) and **90** domains (see `config.yaml` and `--list-engines`).
